...
def query(request, qId):
    ...
    if "difficulty" in request.POST:
    	query.difficulty = request.POST['difficulty']
...
def document(request, qId, docId):
    ...
    content = ""
    with open(settings.DATA_DIR+"/"+document.docId) as f:
    	content = f.read()

    return render_to_response('judgementapp/document.html', {'document': document, 'query': query, 'judgement': judgement, 'content': content.strip()}, 
    	context_instance=RequestContext(request))
...
def judge(request, qId, docId):
	...
	judgement.relevance = relevance
	judgement.comment = comment
	judgement.save()

	return HttpResponseRedirect( '/query/%s' % query.qId)

	#return render_to_response('judgementapp/document.html', {'query':query, 'document': document}, RequestContext(request))
